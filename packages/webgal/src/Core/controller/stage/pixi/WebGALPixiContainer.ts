...
const FILTER_CONFIGS: Record<string, FilterConfig> = {
  ...
  shockwave: {
    priority: FilterPriority.ShockWave,
    create: () => {
      const f = new ShockwaveFilter([1280, 720]);
      f.time = 0;
      return f;
    },
  ...
  radiusAlpha: {
    priority: FilterPriority.RadiusAlpha,
    create: () => {
...
const PROPERTY_CONFIGS: Record<string, PropertyConfig> = {
  ...
  shockwaveFilter: {
    filterName: 'shockwave',
    filterProperty: 'time',
    defaultValue: 0,
  },
  radiusAlphaFilter: {
    filterName: 'radiusAlpha',
