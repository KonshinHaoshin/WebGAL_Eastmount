        ...
        {thinkingOption.options.map((option, index) => {
          ...
          const onClick = async () => {
            ...
            if (option.refuteVideo) {
              ...
              perform.stopFunction = () => { originalStop(); executeJump(); };
              WebGAL.gameplay.performController.arrangeNewPerform(perform, {
                command: 0 as any, commandRaw: '', args: [], content: videoPath,
                sentenceAssets: [], subScene: [], inlineComment: '', isLineBreakHolder: false,
              });
            } else {
              executeJump();
            }
