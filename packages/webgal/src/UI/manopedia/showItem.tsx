...
export const ShowItem: FC = () => {
    ...
    const showItem = useSelector((state: RootState) => state.GUI.showItem);
    const showItemInfo = useSelector((state: RootState) => state.GUI.showItemInfo);
    ...
    useEffect(() => {
        if (showItem && showItemInfo) {
            // 如果有新的物品添加，添加到队列
            const itemId = showItemInfo.itemId || `item-${Date.now()}`;
            const itemImage = showItemInfo.itemImage || './game/Item/ansk/128x128.png';

            addToQueue(itemId, itemImage);
        } else if (!showItem && isVisible) {
